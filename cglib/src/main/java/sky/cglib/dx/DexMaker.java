 ...
 * </ul>
 *
 * <h3>Example: Fibonacci</h3> To illustrate how this API is used, we'll use
 * DexMaker to generate a class equivalent to the following Java source:
 * 
 * <pre>
 *  {@code
 *
 ...
 * bitwise ORing constants from {@link Modifier}. The declare call returns a
...
public final class DexMaker {
	...
	/**
	 * Creates a new {@code DexMaker} instance, which can be used to create a single
	 * dex file.
	 */
	public DexMaker() {}
     ...
     * @return
     */
    public boolean isAlreadyDeclared(MethodId<?, ?> method) {
		TypeDeclaration typeDeclaration = getTypeDeclaration(method.declaringType);
		return typeDeclaration.methods.containsKey(method);
	}
	 ...
	 *            <p>
	 * 			<strong>Warning:</strong> the {@link Modifier#SYNCHRONIZED} flag
	 *            is insufficient to generate a synchronized method. You must also
	 *            use {@link Code#monitorEnter} and {@link Code#monitorExit} to
	 *            acquire a monitor.
	 */
	public Code declare(MethodId<?, ?> method, int flags) {
		...
		if ((flags & ~supportedFlags) != 0) {
			if (flags == 102) {
				throw new IllegalArgumentException("Unexpected flag: " + Integer.toHexString(flags) + " 方法不能私有化(private)");
			} else {
				throw new IllegalArgumentException("Unexpected flag: " + Integer.toHexString(flags) + " 方法不能私有化(private)");
			}
		}
	 ...
	 * @param staticValue
	 *            a constant representing the initial value for the static field,
	 *            possibly null. This must be null if this field is non-static.
	 */
	...
	private ClassLoader generateClassLoader(File result, File dexCache, ClassLoader parent) {
		try {
			return (ClassLoader) Class.forName("dalvik.system.DexClassLoader").getConstructor(String.class, String.class, String.class, ClassLoader.class).newInstance(result.getPath(),
					dexCache.getAbsolutePath(), null, parent);
	 ...
	 * application-private directory. If you pass a world-writable directory like
	 * {@code /sdcard} a malicious app could inject code into your process. Most
	 ...
	 * dexmaker.dexcache} system property. If that exists, it will be used for the
	 * dex cache. If it doesn't exist, this method will attempt to guess the
	 * application's private data directory as a last resort. If that fails, this
	 * method will fail with an unchecked exception. You can avoid the exception by
	 * either providing a non-null value or setting the system property.
	 *
	 * @param parent
	 *            the parent ClassLoader to be used when loading our generated types
	 * @param dexCache
	 *            the destination directory where generated and optimized dex files
	 *            will be written. If null, this class will try to guess the
	...
	public ClassLoader generateAndLoad(ClassLoader parent, File dexCache) throws IOException {
		...
		/*
		 * This implementation currently dumps the dex to the filesystem. It jars the
		 * emitted .dex for the benefit of Gingerbread and earlier devices, which can't
		 * load .dex files directly. TODO: load the dex from memory where supported.
		 */
